...
class LinkedInScraperService {
    ...
    async scrapeProfiles(searchUrl, maxProfiles = 20) {
                ...
                while (profiles.length < maxProfiles && currentPage < maxPages) {
                    ...
                    for (let i = 0; i < profileCards.length && profiles.length < maxProfiles; i++) {
                        try {
                            ...
                            console.log(`⚠️  Failed to extract profile ${i + 1}:`, error.message);
                            continue;
                        }
    ...
    async extractProfileData(card) {
        try {
            ...
            const fullName = await nameElement.evaluate((el) => el.textContent?.trim());
            const profileLink = await nameElement.evaluate((el) => el.href);
            ...
            for (const selector of headlineSelectors) {
                ...
                if (element) {
                    headline = await element.evaluate((el) => el.textContent?.trim());
                    if (headline)
                        break;
                }
            }
